...
async getUsersData(){
        ...
        for (var i = 0; i < res.data.results.length; i++){
            if (citycount[res.data.results[i].place_of_birth_city.value]===undefined) {
                citycount[res.data.results[i].place_of_birth_city.value]=1;
            }
            else {
                citycount[res.data.results[i].place_of_birth_city.value] +=1;
            }
        }
        Object.keys(citycount).forEach(val => {
            newarr.push(val)
        });
        Object.values(citycount).forEach(val => {
            newarr1.push(val)
        });
